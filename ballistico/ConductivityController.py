...
class ConductivityController (object):
	def __init__(self, energies, velocities, n_k_points, n_modes, temperature, volume, folder):
		self.energies = energies
		self.velocities = velocities
		self.n_k_points = n_k_points
		self.n_modes = n_modes
		self.n_phonons = n_k_points * n_modes
		self.folder = folder
	...
	def read_lambd(self, converged=False):
		if converged:
			filename = 'GGG.lambd_final'
		else:
			filename = 'GGG.lambd_initial'
		temperature = str (int (self.temperature))
		filename = self.folder + 'T' + temperature + 'K/' + filename
		lambd = read_file (filename)
		lambd = lambd.reshape (3, self.n_modes, self.n_k_points)
		lambd = lambd.swapaxes (0, 2)
		lambd = lambd.swapaxes (0, 1)
		return lambd
	...
	def calculate_conductivity(self, is_classical=False, post_processing=None, length=None, converged=False):
		...
		lambd = self.read_lambd (converged=converged).reshape (self.n_phonons, 3)
		
		if post_processing == 'matthiesen':
			transmission = self.transmission_matthiesen
		elif post_processing == 'caltech':
			transmission = self.transmission_caltech
		else:
			transmission = self.transmission_infinite
		...
		for alpha in range (3):
			for beta in range (3):
				rates = self.velocities[:, alpha] / (lambd[:, alpha] + THREESHOLD)
				tau_zero = transmission (rates, self.velocities[:, alpha], length)
