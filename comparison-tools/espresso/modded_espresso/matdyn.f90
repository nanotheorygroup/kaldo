...
SUBROUTINE frc_blk(dyn,q,tau,nat,nr1,nr2,nr3,frc,at,bg,rws,nrws,f_of_q,fd)
  ...
  REAL(DP) frc(nr1,nr2,nr3,3,3,nat,nat), tau(3,nat), q(3), arg, &
               ...
               real_totfrc, imag_totfrc, real_eiqr, imag_eiqr
  ...
  FIRST_TIME : IF (first) THEN
    first=.false.
    PRINT *, "FORCESFORMAT q1, q2, q3, n1, n2, n3, na, nb, ",      &
             "alpha, beta, w, q dot r, eiqr-real, eiqr-imag, ",    &
             "totfrc-real, totfrc-imag"
    ...
    PRINT *, "Logic was unchanged except to allow for output and ",&
             "edited code is demarcated by comments near line 1090."
           ...
           DO n2=-nr2_,nr2_
              DO n3=-nr3_,nr3_
                 ...
                 IF (weight .GT. 0.0d0) THEN
                    ...
                    ! BEGIN MODIFIED CODE >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                    imag_eiqr = aimag(eiqr)
                    real_eiqr = real(eiqr)
                    DO ipol=1, 3
                       DO jpol=1, 3
                          totfrc = (frc(m1,m2,m3,ipol,jpol,na,nb)+                     &
                                    f_of_q(ipol,jpol,na,nb))*                          &
                                    CMPLX(COS(arg),-SIN(arg),kind=DP)*                 &
                                    weight
                          real_totfrc = real(totfrc)
                          imag_totfrc = aimag(totfrc)
