...
kaldo = np.load('kaldo-compiled.npy')
matdyn = np.load('matdyn-compiled.npy')
...
for q in q_matdyn:
    ...
    matdyn_q_args = np.prod(np.isclose(matdyn[:, :3], q, rtol=rtoll), axis=1)
    kaldo_q_args  = np.prod(np.isclose(kaldo[:, :3], q), rtol=rtoll, axis=1)
    ...
    for sc in sc_matdyn:
        ...
        if kaldo_total_matches.sum() == 0: # No kALDo entries, store a blank for kALDo
            ...
            # compare data if we see two entries
            for dat in ['qvec', 'sc', 'qr', 'eiqr', 'weight', 'totfrc']:
                if np.unique(kaldo_subarray[dat], axis=-1) == 1:
