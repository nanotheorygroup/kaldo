...
class HarmonicWithQ(Observable):
    ...
    def calculate_eigensystem_unfolded(self, only_eigenvals=False):
        ...
        #
        # ir : number of supercells with weight
        # n_atom_iter : shape of weight matrix = nunitatoms**2
        # n_interactions : shape of force matrix = nunitatoms**2 * 3**2
        #     3**2 factor included for 3 cartesian directions per interaction of atoms
        # debugshape = larger than necessary, then snipped to ir before saving to file
        # debugtype = custom numpy data type
        ir = 0
        debugshape = supercell_replicas.shape[0]
        debugdtype = [("qvec", float, (3)),
                      ("sc", int, (3)),
                      ("qr", float),
                      ("eiqr", complex),
