...
# sheng2_y = np.loadtxt(f'{outfold}/{pathstring}.freqs-sheng2')
...
matdyn_y = np.loadtxt('matdyn/d-gxuglw/FREQ.gp')[:, 1:] / 33.356
matdyn_x = np.arange(matdyn_y.shape[0])/(matdyn_y.shape[0])
for column in range(sheng_y.shape[1]):
    # ax0.scatter(kaldo_x, sheng2_y[:, column], color=set1[3], s=12, zorder=11, alpha=1)
...
phonopy_data = np.loadtxt('phonopy/with_NAC/scaledisp.dat')
...
'''
lines = [# Line2D([],[], lw=0, marker='o', markersize=8, color=set1[3]),
...
ax0.legend(lines, labels, title='Correction', bbox_to_anchor=(0.9, 0.9))
...
fig1.savefig(f'{outfold}/{pathstring}.dispersion-v.png')
