...
# and the lattice constant of the unit cell is 3.566 ?. Other
# input paramters for forconstants object inlcude super cell structure
...
# After computing IFCS using the forceconstants object, we set
# up the phonon object and use it to compute 2nd and 3rd order(phonon)
...
# for 3rd order quantities using TensorFlow (GPU) backend. 
k = 5
...
# Other quatities, such as heat capacity, can also be computed 
# from the phonon object. Meawhile, the visualization of
# phonon properties can also be coustomized by using matplotlib. The
# following line show an example of plotting 
...
# After computing 2nd and 3rd (phonon) order properties, the thermal 
...
# to solve for BTE.Lastly, various of fomrats to store conductivity 
# and mean free path are also available in the Conductivity 
# object (i.e. "formatted" for ASCII fromat data, 
# "numpy" for python numpy array format and "memroy" for 
# quick calculations, no data stored). 
...
# the mean free path from invserion by the group velocities.
...
for alpha in range(3):
  tau_inversion[physical_mode,alpha] = np.abs(np.divide(mean_free_path_inversion[physical_mode,alpha],velocity[physical_mode,alpha]))
