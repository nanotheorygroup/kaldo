...
DEFAULT_ATT3_DEBUG = Path(
    "/data/nwlundgren/rephonopy/.worktrees/gonze-lee-nac-debug-replace/example/nacl-att4/debug"
)
DEFAULT_ATT3_DEBUG_FALLBACK = Path(
    "/data/nwlundgren/rephonopy/.worktrees/gonze-lee-nac-debug-replay/example/nacl-att4/debug"
)
DEFAULT_ATT3_VELOCITY_DEBUG = Path(
    "/data/nwlundgren/rephonopy/.worktrees/gonze-velocity-debug-dump/example/nacl-att4/debug-velocity"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare and plot kALDo vs phonopy frequencies along G->X with 71 points."
    )
    parser.add_argument(
        "--debug-root",
        type=Path,
...
def gather_q_dirs(debug_root: Path) -> list[Path]:
    ...
    expected = [f"q-{i:05d}" for i in range(71)]
...
def load_second_order(folder: str, nac_source: str):
    ...
    _, _, charges = shengbte_io.read_second_order_qe_matrix("../nacl_phonopy/espresso.ifc2")
    if charges is None:
...
def evaluate_path(second_order, q_dirs: list[Path], args: argparse.Namespace) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    for idx, q_dir in enumerate(q_dirs):
        ...
        if status == "FAIL" and first_fail_q_index is None:
            first_fail_q_index = idx
...
def save_plot(kaldo_freqs: np.ndarray, ref_freqs: np.ndarray, kaldo_vels: np.ndarray, ref_vels: np.ndarray, output_path: Path) -> None:
    ...
    ax1.set_xlabel("Path index (G->X, 71 points)")
...
def main() -> None:
    ...
    kaldo_freqs, ref_freqs, kaldo_vels, ref_vels, first_fail_q_index = evaluate_path(second_order, q_dirs, args)
    ...
    print(f"first_fail_q_index={first_fail_q_index}")
