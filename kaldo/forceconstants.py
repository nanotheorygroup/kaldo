...
class ForceConstants:
    ...
    def __init__(
        ...
        self.second = SecondOrder.from_supercell(
            atoms,
        ...
        self.third = ThirdOrder.from_supercell(
            atoms, supercell=third_supercell, grid_type="C", folder=folder
        )
    ...
    def from_folder(
        ...
        forceconstants = {"atoms": atoms, "supercell": supercell, "folder": folder}
    ...
    def unfold_third_order(self, reduced_third=None, distance_threshold=None):
        ...
        if (
            (self.atoms.cell[0, 0] / 2 < distance_threshold)
        ...
        dxij_reduced = wrap_coordinates(
            ...
            self.third.replicated_atoms.cell,
            replicated_cell_inv,
        )
        indices = np.argwhere(
    ...
    def elastic_prop(self):
        ...
        w_mu = np.abs(np.array(h0._eigensystem[0, :])) ** (
            0.5
        )  # optical frequencies (w/(2*pi) = f) in THz
    ...
    def _calculate_displacement(atoms, initial_structure):
        ...
        return find_mic(disp.reshape(-1, 3), atoms.cell)[0].reshape(
            initial_structure.positions.shape
        )
    ...
    def _calculate_sigma(md_forces, harmonic_forces):
        return np.sqrt(mean_squared_error(md_forces, harmonic_forces)) / np.std(
            md_forces
        )
    ...
    def sigma2_tdep_MD(
        ...
        primitive_file="infile.ucposcar",
        supercell_file="infile.ssposcar",
        md_run="dump.xyz",
    ):
        """
        ...
        displacements = [
            ForceConstants._calculate_displacement(atoms, initial_structure)
            for atoms in full_MD_traj
        ]
        force_harmonic = [
            ForceConstants._calculate_harmonic_force(disp, second_order_fc)
            for disp in displacements
        ]
        sigma_values = [
            ForceConstants._calculate_sigma(atoms.get_forces(), harm_force)
