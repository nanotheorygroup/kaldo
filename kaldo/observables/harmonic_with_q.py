...
class HarmonicWithQ(Observable):
    ...
    def calculate_nonanalytical_corrections(self):
        avec = self.second.atoms.cell[:, 0]
        epsilon = self.second.atoms.info['epsilon']
        zeff = self.second.atoms.get_array('charges')
        nunit = self.second.atoms.positions.shape[0]
        cell_r = self.second.atoms.cell
        cell_g = self.second.cell_inv
        atoms = self.second.atoms
        distances = atoms.positions[:, None, :] - atoms.positions[None, :, :]
        dyn_g = np.zeros((nunit, 3, nunit, 3))

        gmax = 14.0
        alpha = (2 * np.pi * units.Bohr / np.linalg.norm(avec)) ** 2
        geg = gmax * 4 * alpha

        # make a grid with these
        ncell_g = int(np.sqrt(geg) / np.linalg.norm(cell_g, axis=1)) + 1
        gcells = np.ones((ncell_g, 3))
        #
        # for gvec in gcells:
        ...
        #                 auxi += zjg * np.exp(gr)
            ...
            #     gvecp = gvec + k(ikpt, :)
            #     geg = g.T @ epsilon @ gvec
            #     geg_normalized = geg / (4 * alpha)
            #     if geg>0 and geg_normalized < gmax:
            #         exp_g = np.exp( -geg_normalized ) / geg
            #         dgeg = ( epsilon + epsilon.T ) @ gvec
            #         for na in range(nunit):
            #             zig = gvec @ zeff[na, :, :]
            #             for nb in range(nunit):
            #                 gr = gvec @ distances[na, nb, :]
            #                 zjg = gvec @ zeff[nb, :, :]
                            ...
                            #     for beta in 3
                            #         jdim = (jat - 1) * 3 + beta
                            #         dyn_g[ikpt, :, :] += exp_g * zig(alpha) * zjg(beta) * np.exp(gr)
                            #
                            #         for index in 3:
                            #             ddyn_g[ikpt, :, :, index] += exp_g * np.exp(gr) *
                            #                 zjg
                            #     (zjg(jpol) * zeff(iat, i, ipol) +
