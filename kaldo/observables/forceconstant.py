...
def chi(qvec, list_of_replicas, cell_inv):
    """Return Fourier phases for Cartesian translations at a reduced q point.

    ``list_of_replicas @ cell_inv`` converts the stored Cartesian vectors back
    to integer primitive translations, so the result is
    ``exp(2*pi*i*q.R)``. The historical function name is retained because
    anharmonic projection code imports it directly.
    """
...
class ForceConstant(Observable):
    ...
    @property
    def positions(self):
        """Cartesian primitive-cell basis positions in angstrom."""
        return self.atoms.positions

    @property
    def cell_inv(self):
        """Inverse primitive-cell matrix in the ASE row-vector convention."""
    ...
    def replicated_cell_inv(self):
        """Inverse Cartesian cell of :attr:`replicated_atoms`."""
