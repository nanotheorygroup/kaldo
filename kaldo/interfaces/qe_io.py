...
def read_second_order_qe_matrix(filename):
    ...
    The numerical body keeps the original kALDo layout and converts QE's
    Ry/bohr² values to eV/angstrom². Macroscopic tensors are returned without
    modification because the NAC controller must preserve QE's convention.
    ...
    -------
        ...
        charges : np.ndarray or None
            Dielectric tensor followed by one Born tensor per atom. ``None``
            indicates that the q2r header had its macroscopic-data flag off.
    """
...
# QE 7.5 constants.f90.  q2r stores masses in electron-mass / 2 units.
...
class Q2RHeader:
    """Structural and macroscopic metadata preserved by formatted q2r IFCs."""
...
def read_q2r_header(path: str | Path) -> Q2RHeader:
    ...
    in atomic mass units. The returned line indices allow callers to preserve
    the original force-constant body byte-for-byte.
    """
    source = Path(path)
