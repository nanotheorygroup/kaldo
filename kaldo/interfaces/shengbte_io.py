...
def import_control_file(control_file):
    ...
    if masses is None:
        ...
        atoms = Atoms(list_of_elem, positions=positions, cell=cell, pbc=[1, 1, 1])
    else:
        list_of_masses = []
        for i in range(len(types)):
            list_of_elem.append(elements[types[i] - 1])
...
def save_second_order_matrix(phonons):
    ...
    second_order = second_order.reshape(
        (n_atoms_unit_cell, 3, n_replicas, n_atoms_unit_cell, 3)
    )
            ...
            for l0 in range(n_replicas):
                for i1 in range(n_atoms_unit_cell):
                    for l1 in range(n_replicas):
                        file.write(
                            str(l0 + i0 * n_replicas + 1)
                            + "  "
                            + str(l1 + i1 * n_replicas + 1)
                            + "\n"
                        )
                        if l0 == 0:
...
def save_third_order_matrix(phonons):
    ...
    n_replicas = phonons.forceconstants.n_replicas
    third_order = phonons.forceconstants.third.value.reshape(
        (
            n_replicas,
            n_in_unit_cell,
            3,
            ...
            for i_1 in range(n_in_unit_cell):
                for n_2 in range(n_replicas):
                    for i_2 in range(n_in_unit_cell):
                        three_particles_interaction = third_order[
                            0, i_0, :, n_1, i_1, :, n_2, i_2, :
                        ]
                        ...
                        if (np.abs(three_particles_interaction) > 1e-9).any():
                            ...
                            file.write("\n  " + str(block_counter))
                            rep_position = (
                                phonons.forceconstants.second.list_of_replicas[n_1]
                            )
                            file.write(
                                "\n  "
                                + str(rep_position[0])
                            ...
                            rep_position = (
                                phonons.forceconstants.second.list_of_replicas[n_2]
                            )
                            file.write(
                                "\n  "
                                + str(rep_position[0])
...
def create_control_file_string(phonons, is_espresso=False):
    ...
    string += (
        "\tscell(:)="
        + str(phonons.supercell[0])
        + " "
        + str(phonons.supercell[1])
        + " "
        + str(phonons.supercell[2])
        + "\n"
    )
    string += "&end\n"
    string += "&parameters\n"
