...
def _split_index(index, nx, ny, nz):
    """Split a zero-based VASP IFC index into cell and atom indices.

    VASP flattens the index as
    ``(((iatom * nz + iz) * ny + iy) * nx + ix)``.
    """
...
def read_second_order_matrix(filename, supercell):
    ...
    Only rows whose first atom lies in the origin replica are retained, which
    matches kALDo's compact primitive-to-supercell IFC representation.
    ...
    with open(filename, "r") as file:
        ...
        second_order = np.zeros(
            (n_unit_atoms, 3, supercell[0], supercell[1], supercell[2], n_unit_atoms, 3)
        )
        ...
        while line:
            ...
            i_ix, i_iy, i_iz, i_iatom = _split_index(
                i, supercell[0], supercell[1], supercell[2]
            )
            j_ix, j_iy, j_iz, j_iatom = _split_index(
                j, supercell[0], supercell[1], supercell[2]
            )
