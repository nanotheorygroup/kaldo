...
Correctness is established by representation invariants and an independent
finite-difference first moment.  Absolute values generated by kALDo are kept
separately as regression snapshots; they are not external conductivity truth.
"""
...
def _finite_difference_pair_kernel(second, direction, step=1.0e-5):
    """Differentiate an independent cubic minimum-image Fourier sum.

    The production interpolation plan is deliberately not used here.  For
    this compact cubic fixture the nearest displacement can be constructed
    directly by wrapping each fractional atom-pair difference.  Dividing the
    centered difference of ``exp(-i k.r)`` by ``i`` gives the real-space
    first moment consumed by the amorphous heat-flux operator.
    """
    fractional = second.atoms.get_scaled_positions(wrap=False)
    pair_fractional = fractional[np.newaxis, :, :] - fractional[:, np.newaxis, :]
    pair_fractional -= np.rint(pair_fractional)
    displacement = pair_fractional @ np.asarray(second.atoms.cell)
    values = np.asarray(second.dynmat)[0, :, :, 0, :, :]

    matrices = []
    for wavevector in (-step, step):
        phase = np.exp(-1j * wavevector * displacement[..., direction])
        matrices.append(np.einsum("ij,iajb->iajb", phase, values))
    derivative = (matrices[1] - matrices[0]) / (2j * step)
    return derivative.reshape(3 * len(fractional), 3 * len(fractional)).real
...
def test_gamma_frequencies_hide_but_heat_flux_exposes_legacy_image_bug():
    ...
    np.testing.assert_allclose(
        legacy_change,
        METADATA["regression_snapshots"]["legacy_origin_flux_relative_change"],
        rtol=2e-10,
        atol=0.0,
    )


def test_heat_flux_first_moment_matches_independent_finite_difference():
    """The analytic pair kernel equals a direct Cartesian phase derivative."""
    forceconstants = _load_forceconstants()
    harmonic = _harmonic(forceconstants, "auto")
    for direction in range(3):
        actual = np.asarray(harmonic.calculate_dynmat_derivatives(direction))
        expected = _finite_difference_pair_kernel(forceconstants.second, direction)
        np.testing.assert_allclose(actual, expected, rtol=2e-9, atol=2e-9)


def test_ifc3_fourier_gauge_is_origin_invariant_off_gamma():
...
def qhgk_origin_results():
    """Evaluate corrected and diagnostic QHGK routes once for this module."""
    reference_fc = _load_forceconstants()
    shifted_fc = _load_forceconstants()
    _move_compact_origin(shifted_fc)
    ...
    return {
        ...
        "shifted_diffusivity": shifted_diffusivity,
        "legacy_reference_tensor": legacy_reference_tensor,
        "legacy_shifted_tensor": legacy_shifted_tensor,
    }
...
def test_qhgk_transport_is_origin_invariant_and_legacy_route_is_not(
    ...
    results = qhgk_origin_results

    np.testing.assert_allclose(
        results["shifted"].bandwidth,
        results["reference"].bandwidth,
        rtol=2e-12,
        atol=2e-12,
    )
    np.testing.assert_allclose(
        results["shifted_diffusivity"],
        results["reference_diffusivity"],
        rtol=2e-10,
        atol=2e-12,
    )
    np.testing.assert_allclose(
        results["shifted_tensor"],
        results["reference_tensor"],
        rtol=2e-10,
        atol=2e-12,
    )

    reference_trace = float(np.trace(results["legacy_reference_tensor"]))
    shifted_trace = float(np.trace(results["legacy_shifted_tensor"]))
    legacy_change = abs(shifted_trace - reference_trace) / abs(reference_trace)
    assert legacy_change > 0.01


def test_qhgk_numeric_regression_snapshot(qhgk_origin_results):
    """Record numerical drift; this self-generated value is not ground truth."""
    results = qhgk_origin_results
    np.testing.assert_allclose(
        results["reference_tensor"],
        np.asarray(METADATA["regression_snapshots"]["qhgk_tensor_W_mK"]),
        rtol=2e-9,
        atol=2e-12,
    )
    ...
    np.testing.assert_allclose(
        legacy_change,
        METADATA["regression_snapshots"]["legacy_origin_qhgk_trace_relative_change"],
        rtol=0.0,
