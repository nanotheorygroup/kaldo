...
def test_parse_pair_valid():
    assert parse_pair('(1+2j)') == complex(1, 2)
    assert parse_pair('(-3-4j)') == complex(-3, -4)
    assert parse_pair('(5+0j)') == complex(5, 0)
    assert parse_pair('(0+0j)') == complex(0, 0)
...
def test_parse_pair_scientific_notation():
    assert parse_pair('(1e3+2e3j)') == complex(1e3, 2e3)
    assert parse_pair('(-1e-3-2e-3j)') == complex(-1e-3, -2e-3)

def test_parse_pair_missing_real_or_imaginary():
    assert parse_pair('(0+2j)') == complex(0, 2)
    assert parse_pair('(1+0j)') == complex(1, 0)

def test_parse_pair_uppercase_j():
    assert parse_pair('(1+2J)'.replace('J', 'j')) == complex(1, 2)
