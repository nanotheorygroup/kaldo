...
def phonons():
    ...
    forceconstants = ForceConstants.from_folder(folder="kaldo/tests/si-crystal-mix-cell", supercell=[6, 6, 6], third_supercell=[3, 3, 3], format="lammps")
    phonons = Phonons(
        folder='data',
...
def test_sc_conductivity(phonons):
    ...
    np.testing.assert_approx_equal(cond, 262, significant=3)
...
def test_qhgk_conductivity(phonons):
    ...
    np.testing.assert_approx_equal(cond, 240, significant=3)
