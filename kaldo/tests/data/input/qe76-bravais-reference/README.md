...
The checked-in `<case>.fc`, `<case>.without-nac.fc`, `<case>.on.dyn`, and
`<case>.off.dyn` files are reference outputs, not scratch data. Tests parse the
real q2r header through `kaldo.interfaces.qe_io` and compare the Python
rigid-ion tensor directly with QE's readable matdyn matrix difference.
